...
class SP500DataLoader_():
    ...
    def get_raw_prices(self, start_date: tuple, end_date: tuple, interval='1d', column='Adj Close', save_as_h5=False, save_as_csv=False):
        ...
        if save_as_csv:
            self.write_on_disk(self.raw_prices, "S&P500-raw_prices_all.csv")
        if save_as_h5:
            self.write_on_disk(self.raw_prices, "S&P500-raw_prices_all.h5")
    ...
    def get_raw_returns(self, start_date: tuple, end_date: tuple, interval='1d', column='Adj Close', save_as_h5=False, save_as_csv=False):
        ...
        self.raw_returns = (self.raw_returns).diff()
        self.raw_returns = self.raw_returns.iloc[1:]  # removes first row which is NaN after diff()

        if save_as_csv:
            self.write_on_disk(self.raw_returns, "S&P500-raw_returns_all.csv")
        if save_as_h5:
            self.write_on_disk(self.raw_returns, "S&P500-raw_returns_all.h5")
    ...
    def get_cleaned_prices(self, start_date: tuple, end_date: tuple, interval='1d', column='Adj Close', save_as_h5=False, save_as_csv=False):
        ...
        if save_as_csv:
            self.write_on_disk(self.self.cleaned_prices, "S&P500-cleaned_prices_all.csv")
        if save_as_h5:
            self.write_on_disk(self.self.cleaned_prices, "S&P500-cleaned_prices_all.h5")
    ...
    def get_cleaned_returns(self, start_date: tuple, end_date: tuple, interval='1d', column='Adj Close', save_as_h5=False, save_as_csv=False):
        ...
        self.cleaned_returns = self.cleaned_returns.diff()
        self.cleaned_returns = self.cleaned_returns.iloc[1:]  

        if save_as_csv:
            self.write_on_disk(self.cleaned_returns, "S&P500-cleaned_returns_all.csv")
        if save_as_h5:
            self.write_on_disk(self.cleaned_returns, "S&P500-cleaned_returns_all.h5")
    ...
    def get_last_cleaned_prices(self, save_as_h5=False, save_as_csv=False):
        ...
        if save_as_csv:
            self.write_on_disk(self.cleaned_prices, "S&P500-cleaned_prices_all.csv")
        if save_as_h5:
            self.write_on_disk(self.cleaned_prices, "S&P500-cleaned_prices_all.h5")
